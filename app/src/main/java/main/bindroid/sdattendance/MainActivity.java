...
public class MainActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mRegion = new Region(Globals.REGION + "_",
				Globals.PROXIMITY_UUID, Globals.MAJOR, Globals.MINOR);
				...
				.setMonitoringListener(new BeaconManager.MonitoringListener() {
					...
					public void onEnteredRegion(final Region region,
						...
						((TextView) findViewById(R.id.status))
								.setText(R.string.status_entered_region);
					}
					...
					public void onExitedRegion(final Region region) {
						mStatusText = getString(R.string.status_entered_region);
						((TextView) findViewById(R.id.status))
								.setText(R.string.status_exited_region);
					}
				});

		if (getIntent().hasExtra(EXTRA_STATUS_TEXT)
				&& getIntent().getStringExtra(EXTRA_STATUS_TEXT) != null
				&& getIntent().getStringExtra(EXTRA_STATUS_TEXT).length() > 0) {
		...
		mSwitch.setChecked(mPreferences.getBoolean(
				Globals.PREFERENCE_SERVICE_STARTED, false));
	...
	private void startBeaconService() {
		if (checkCredentials() && checkBluetooth()) {
			...
			editor.putString(Globals.PREFERENCE_EMAIL, mEmail.getText()
					.toString());
			editor.putString(Globals.PREFERENCE_PASSWORD, mPassword.getText()
					.toString());
			editor.commit();
